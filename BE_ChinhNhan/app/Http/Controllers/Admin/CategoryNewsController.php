    ...
    public function update(UpdateCategoryNewsRequest $request, string $cat_id)
    {
        try {
            ...
            DB::transaction(function () use ($request, $data, $newsCategory) {
                $picture = updateImage($request->picture,$newsCategory->picture,'uploads/category');
                $newsCategory->update([
                ...
                $newsCategory->newsCategoryDesc()->update(
                    ...
                    [
                        'cat_name' => $data['cat_name'] ?? optional($newsCategory->newsCategoryDesc)->cat_name,
                        'description' => $data['description'] ?? optional($newsCategory->newsCategoryDesc)->description,
                        'friendly_title' => $data['friendly_title'] ?? optional($newsCategory->newsCategoryDesc)->friendly_title,
                        'metakey' => $data['metakey'] ?? optional($newsCategory->newsCategoryDesc)->metakey,
                        'metadesc' => $data['metadesc'] ?? optional($newsCategory->newsCategoryDesc)->metadesc,
                    ]
                );
            });
